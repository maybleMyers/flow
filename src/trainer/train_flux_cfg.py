...
class TrainingConfig:
    ...
    save_every: int
    save_folder: str
    cfg_max_strength: float  # Max random CFG strength
...
def is_conditional_batch(str_list):
    """
    Creates a boolean tensor indicating which strings in a list are non-empty.
    True for conditional (non-empty), False for unconditional (empty).
    """
    return torch.tensor([s != "" for s in str_list], dtype=torch.bool)
...
def train_flux(rank, world_size, debug=False):
    ...
    while True:
        ...
        for counter, data in tqdm(
            ...
            is_conditional = is_conditional_batch(caption).to(rank, non_blocking=True)
            ...
            for mb_i in tqdm(range(num_cache_iters), desc=f"preparing latents, Rank {rank}", position=rank):
                with torch.no_grad(), torch.autocast(device_type="cuda", dtype=torch.bfloat16):
                    ...
                    # Pre-compute negative embeddings once and cache them
                    if negative_embeds is None:
                        neg_t5_inputs = t5_tokenizer(
                            [""] * training_config.cache_minibatch, padding="max_length", max_length=model_config.t5_max_length,
                            truncation=True, return_tensors="pt"
                        ).to(t5.device)
                        neg_t5_embed = t5(neg_t5_inputs.input_ids, neg_t5_inputs.attention_mask).to("cpu", non_blocking=True)
                        
                        neg_clip_inputs = clip_tokenizer(
                            [""] * training_config.cache_minibatch, padding="max_length", max_length=model_config.clip_max_length,
                            truncation=True, return_tensors="pt"
                        ).to(clip.device)
                        neg_clip_embed = clip(input_ids=neg_clip_inputs.input_ids).pooler_output.to("cpu", non_blocking=True)

                        negative_embeds = (neg_t5_embed, neg_t5_inputs.attention_mask, neg_clip_embed)
            ...
            neg_t5_embed, neg_t5_mask, neg_clip_embed = negative_embeds
            ...
            for tmb_i in tqdm(range(num_train_iters), desc=f"minibatch training, Rank {rank}", position=rank):
                ...
                # Generate random CFG strengths for the minibatch
                random_cfg_strength = torch.rand(end_idx - start_idx, device=rank) * training_config.cfg_max_strength
                
                # For unconditional samples, guidance is 0
                guidance_values = torch.where(is_conditional[start_idx:end_idx], random_cfg_strength, 0.0).to(torch.bfloat16)

                with torch.autocast(device_type="cuda", dtype=torch.bfloat16):
                    # Get unconditional prediction for CFG distillation target
                    with torch.no_grad():
                        pred_neg = model(
                            img=noisy_latents[start_idx:end_idx],
                            img_ids=image_pos_id[start_idx:end_idx],
                            txt=neg_t5_embed[start_idx:end_idx].to(rank, non_blocking=True),
                            txt_ids=text_ids[start_idx:end_idx],
                            txt_mask=neg_t5_mask[start_idx:end_idx].to(rank, non_blocking=True),
                            y=neg_clip_embed[start_idx:end_idx].to(rank, non_blocking=True),
                            timesteps=input_timestep[start_idx:end_idx],
                            guidance=torch.zeros_like(guidance_values), # Guidance is 0 for uncond
                        )

                    # Create the distilled CFG target
                    cfg_target = pred_neg + guidance_values.view(-1, 1, 1) * (target[start_idx:end_idx] - pred_neg)
                    
                    # Select target: cfg_target for conditional, original target for unconditional
                    final_target = torch.where(is_conditional[start_idx:end_idx].view(-1, 1, 1), cfg_target, target[start_idx:end_idx])
                    ...
                    pred = model(
                        ...
                        guidance=guidance_values, # Pass the random (or zeroed) guidance values
                    )
                    
                    loss = ((pred - final_target) ** 2).mean(dim=(1, 2))
            ...
            del acc_t5_embeddings, noisy_latents, acc_latents, acc_clip_embeddings, final_target, cfg_target, pred_neg
            torch.cuda.empty_cache()
