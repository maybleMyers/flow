...
# MODIFICATION: Added CLIPTokenizer and CLIPTextModel
from transformers import T5Tokenizer, CLIPTokenizer, CLIPTextModel
import wandb

from src.dataloaders.dataloader import TextImageDataset
# MODIFICATION: Changed model import to Flux
from src.models.flux.model import Flux, flux_params
# MODIFICATION: Changed sampling import to denoise_cfg for Flux
from src.models.flux.sampling import get_noise, get_schedule, denoise_cfg
...
class InferenceConfig:
    ...
    t5_max_length: int
    # MODIFICATION: Added clip_max_length for inference
    clip_max_length: int
...
class ModelConfig:
    ...
    # MODIFICATION: Changed chroma_path to flux_path
    flux_path: str
    vae_path: str
    t5_path: str
    t5_config_path: str
    ...
    t5_to_8bit: bool
    t5_max_length: int
    # MODIFICATION: Added CLIP model and tokenizer paths
    clip_path: str
...
# MODIFICATION: Updated inference_wrapper for Flux
def inference_wrapper(
    model,
    ae,
    t5_tokenizer,
    t5,
    # MODIFICATION: Added clip tokenizer and model
    clip_tokenizer,
    clip,
    seed: int,
    steps: int,
    ...
    image_dim=(512, 512),
    t5_max_length=512,
    # MODIFICATION: Added clip_max_length
    clip_max_length=77,
):
    #############################################################################
    # test inference
    ...
    with torch.no_grad():
        with torch.autocast(device_type="cuda", dtype=torch.bfloat16):
            ...
            # MODIFICATION: Generate CLIP embeddings
            clip.to(rank)
            ...
            # MODIFICATION: Call denoise_cfg with both T5 and CLIP embeddings
...
def train_flux(rank, world_size, debug=False):
    ...
    # load model
    with torch.no_grad():
        # MODIFICATION: load flux and enable grad
        ...
        # MODIFICATION: Load CLIP model and tokenizer
        ...
        for counter, data in tqdm(
            ...
            if counter % training_config.change_layer_every == 0:
                ...
                trained_layer_keywords = (
                    ...
                    # MODIFICATION: Added 'vector_in' for CLIP embeddings
            ...
            acc_latents = []
            acc_t5_embeddings = []
            # MODIFICATION: Added accumulator for CLIP embeddings
            acc_clip_embeddings = []
            acc_mask = []
            for mb_i in tqdm(
                range(
                ...
                with torch.no_grad(), torch.autocast(
                    ...
                    # MODIFICATION: Generate and cache CLIP embeddings
                    clip_inputs = clip_tokenizer(
                        caption[
                            mb_i
                            * training_config.cache_minibatch : mb_i
                        ...
                        padding="max_length",
                        max_length=model_config.clip_max_length,
                        truncation=True,
                        return_tensors="pt"
                    ).to(clip.device)
                    clip_embed = clip(clip_inputs.input_ids).pooler_output.to(
                        "cpu", non_blocking=True
                    )
            ...
            # MODIFICATION: Concatenate CLIP embeddings
            ...
            acc_t5_embeddings.requires_grad_(True)
            # MODIFICATION: Set requires_grad for CLIP embeddings
            acc_clip_embeddings.requires_grad_(True)
            ...
            for tmb_i in tqdm(
                ...
                with torch.autocast(device_type="cuda", dtype=torch.bfloat16):
                    # MODIFICATION: Updated model call to include CLIP embeddings (y)
                    pred = model(
                        ...
                        txt=acc_t5_embeddings[tmb_i * mb : tmb_i * mb + mb].to(
                            rank, non_blocking=True
                        ),
            ...
            # MODIFICATION: Added acc_clip_embeddings to be deleted
            ...
            if (counter + 1) % inference_config.inference_every == 0:
                ...
                for prompt in preview_prompts:
                    # MODIFICATION: Updated inference_wrapper call for Flux
                    images_tensor = inference_wrapper(
                        ...
                        clip_max_length=inference_config.clip_max_length,
                    )
                ...
                for extra_inference in extra_inference_config:
                    for prompt in preview_prompts:
                         # MODIFICATION: Updated inference_wrapper call for Flux for extra inference
                        images_tensor = inference_wrapper(
                            model=model,
                            ae=ae,
                            t5_tokenizer=t5_tokenizer,
                            ...
                            clip_max_length=extra_inference.clip_max_length,
                        )
            ...
            acc_latents = []
            acc_t5_embeddings = []
            # MODIFICATION: Clear clip embeddings accumulator
            acc_clip_embeddings = []
            global_step += 1
